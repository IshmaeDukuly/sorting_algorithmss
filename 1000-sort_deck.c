...
int get_card_position(deck_node_t *node)
{
	...
	val = (*node).card->val[0] - '0';
	if (val < 50 || val > 57)
	{
		if (_strcmp((*node).card->val, "Ace") == 1)
			val = 1;
		else if (_strcmp((*node).card->val, "10") == 1)
			val = 10;
		else if (_strcmp((*node).card->val, "Jack") == 1)
			val = 11;
		else if (_strcmp((*node).card->val, "Queen") == 1)
			val = 12;
		else if (_strcmp((*node).card->val, "King") == 1)
			val = 13;
	}
