#include "sort.h"
#include <stdio.h>
/**
 *_calloc - this is the calloc function
 *@nmemb: The numbers of elem
 *@size: The bit size of each ele
 *Return: The pointer to memory assign
 */
void *_calloc(unsigned int nmemb, unsigned int size)
{
	unsigned int m = 0;
	char *p;

	if (nmemb == 0 || size == 0)
		return ('\0');
	p = malloc(nmemb * size);
	if (p == NULL)
		return ('\0');
	for (m = 0; m < (nmemb * size); m++)
		p[m] = '\0';
	return (p);
}
/**
 *merge - To merge
 *@arr: one from starting to the middle
 *@tmp: This is an array used in merge, was created outside to
 *reducing the system calls
 *@start: 1st element position
 *@mid: The array middle
 *@end: last element posit
 **/
void merge(int *arr, int *tmp, int start, int mid, int end)
{
	/* The sizes and the temp arrays */
	int size_left = mid - start + 1, size_right = end - mid;
	int *array_left, *array_right;
	/* countss */
	int left, right, m = 0;

	array_left = &tmp[0];
	array_right = &tmp[size_right];
	for (left = 0; left < size_left; left++)
		array_left[left] = arr[start + left];
	for (right = 0; right < size_right; right++)
		array_right[right] = arr[mid + 1 + right];
	left = 0, right = 0, m = start;
	/* merging tmp arrays into the main array*/
	while (left < size_left && right < size_right)
	{
		if (array_left[left] <= array_right[right])
			arr[m] = array_left[left], left++;
		else
			arr[m] = array_right[right], right++;
		m++;
	}
	/* merging the remaining left an array into the main array*/
	while (left < size_left)
		arr[m] = array_left[left], left++, m++;
	/* merging remaining right an array into to the main array*/
	while (right < size_right)
		arr[m] = array_right[right], right++, m++;
	printf("Merging...\n");
	printf("[left]: ");
	print_array(array_left, left);
	printf("[right]: ");
	print_array(array_right, right);
	printf("[Done]: ");
	print_array(&arr[start], left + right);
}
/**
 *mergesort - The function that sorts an array of ints
 *orderly using the Merge sort algorithm
 *@array: array of integers
 *@tmp: The temp array used in merge, was created outside to
 * reducing the system calls
 *@start: The 1st element position
 *@end: last element pos
 *Return: Always return void
 */
void mergesort(int *array, int *tmp, int start, int end)
{
	int mid;

	mid = (start + end) / 2;
	if ((start + end) % 2 == 0)
		mid--;
	if (mid >= start)
	{
		mergesort(array, tmp, start, mid);
		mergesort(array, tmp, mid + 1, end);
		merge(array, tmp, start, mid, end);
	}
}
/**
 *merge_sort - The function that sorts an array of ints
 *orderly using the Merge sort algorithm
 *@size: The size of list
 *@array: AN array of ints
 *Return: Alwaysreturn the void
 */
void merge_sort(int *array, size_t size)
{
	int *tmp;

	if (!array || size < 2)
		return;
	tmp = _calloc(size, sizeof(int));
	mergesort(array, tmp, 0, size - 1);
	free(tmp);
}
